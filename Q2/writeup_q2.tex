...
\section*{Setup}
The Lecture-3 training script was adapted by swapping the predictor from \texttt{RandomForestClassifier}
to \texttt{MLPClassifier} and the dataset from IRIS to MNIST (\texttt{fetch\_openml("mnist\_784")}, a
6{,}000-sample subset, pixels scaled to $[0,1]$). Six runs were logged to the \texttt{mnist-mlp}
experiment, varying two hyperparameters: \texttt{hidden\_layer\_sizes} and \texttt{learning\_rate\_init}.
The six-run comparison table is provided separately as \texttt{comparison.png}.
